/**
 * AWS Lambda Function: announcement-api
 * Green Valley Public School, Jalandhar
 * Handles school circulars & notices in Amazon DynamoDB
 */
...
const TABLE_NAME = process.env.ANNOUNCEMENTS_TABLE || 'GVPS_Announcements';
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      const ancId = body.id || `ANC-${Date.now()}`;
      const item = { ...body, id: ancId, createdAt: new Date().toISOString() };
