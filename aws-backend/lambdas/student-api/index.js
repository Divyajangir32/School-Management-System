/**
 * AWS Lambda Function: student-api
 * Green Valley Public School, Jalandhar
 * Handles CRUD operations for students in Amazon DynamoDB
 */
...
const TABLE_NAME = process.env.STUDENTS_TABLE || 'GVPS_Students';
  ...
  try {
    ...
    if (method === 'GET') {
      if (id) {
        ...
        if (!result.Item) {
          return { statusCode: 404, headers, body: JSON.stringify({ message: 'Student not found' }) };
        }
    ...
    if (method === 'POST') {
      ...
      if (!body.name || !body.class) {
        return { statusCode: 400, headers, body: JSON.stringify({ message: 'Name and class are required' }) };
      }
